...
--
-- V11 changes vs naive concatenation:
...
--            Eliminates write-conflict between direct field mutation and ledger.
--   [V11-02] insights.events (V8 raw analytics stream) and platform.events (V9
--            event bus) are DISTINCT tables — kept separate as they serve
--            different consumers (analytics vs. integration/webhooks).
--   [V11-03] catering.lead_notes.author column DROPPED — V8 marked it
--            deprecated for V9. V11 finalises the migration.
--   [V11-04] platform.usage_events and platform.usage_ledger UNIFIED into
--            platform.usage_events (raw metered events) and
--            platform.usage_ledger (rolled-up billing periods). Both kept;
--            schema and NOT NULL constraints normalised.
--   [V11-05] platform.events gains event_version, processed, processed_at
--            columns natively (from V10 ALTER ADD COLUMN pattern).
--   [V11-06] tenant.subscriptions gains updated_at for audit trail consistency.
--   [V11-07] customer.identities gains updated_at and index on identity_value.
--   [V11-08] platform.api_keys gains UNIQUE constraint on key_hash (V10).
--   [V11-09] insights.customer_segments gains updated trigger support via
--            updated_at column consistency.
--   [V11-10] inventory schema added to updated_at trigger loop.
--   [V11-11] Updated_at trigger loop extended to cover: inventory, insights
--            (customer_segments), tenant (subscriptions).
...
-- V8 → V11: +20 tables, +1 schema (inventory), +1 view
--   author column dropped from catering.lead_notes (V11-03)
--   inventory.items.current_stock removed (V11-01)
--   platform.events: event_version, processed, processed_at added natively
--   All V9/V10 guards preserved with IF NOT EXISTS for idempotency
-- ============================================================
