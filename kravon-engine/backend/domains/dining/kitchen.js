  ...
  const [dineInResult, queueResult] = await Promise.all([
    query(
      `SELECT
       ...
       LEFT JOIN LATERAL (
         SELECT json_agg(json_build_object('name', oi2.item_name, 'qty', oi2.quantity, 'note', oi2.special_note)) AS items_agg
    ...
    query(
      `SELECT
         ...
         json_agg(
           json_build_object('name', oi.item_name, 'qty', oi.quantity, 'note', oi.special_note)
           ORDER BY oi.id
         ) AS items
